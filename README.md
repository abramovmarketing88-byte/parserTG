...
Проект подготовлен для запуска на [Railway](https://railway.app) прямо из GitHub. В репозитории есть всё необходимое:

| Файл | Назначение |
|------|-------------|
| `Procfile` | Стартовая команда: Streamlit на порту `$PORT`, адрес `0.0.0.0` |
| `requirements.txt` | Зависимости Python (Streamlit, Telethon) |
| `.python-version` | Версия Python 3.11 для сборки на Railway |
| `.streamlit/config.toml` | Режим headless для облака |

**Пошагово:**

1. **Залить код на GitHub**
   - Создайте репозиторий (например `telegram-cloud-scraper`).
   ...
   ```
   Убедитесь, что в коммит не попали `.env` и `*.session` (они в `.gitignore`).

2. **Подключить репозиторий к Railway**
   - Откройте [railway.app](https://railway.app) и войдите (лучше через GitHub).
   - **New Project** → **Deploy from GitHub repo**.
   - Выберите нужный репозиторий и ветку (обычно `main`).
   - Railway сам подхватит `Procfile` и начнёт сборку.

3. **Выдать публичный URL**
   - В проекте откройте ваш сервис (карточка с приложением).
   - Вкладка **Settings** → блок **Networking** → **Generate Domain**.
   - Появится ссылка вида `https://ваш-проект.up.railway.app` — по ней будет открываться сайт.

4. **Переменные окружения**
   - Добавлять **не нужно**: API_ID, API_HASH и сессию пользователи вводят в веб-интерфейсе в боковой панели.

После деплоя (обычно 1–2 минуты) приложение будет доступно по сгенерированному домену. При новых пушах в `main` Railway будет пересобирать и перезапускать приложение автоматически.

---

<details>
<summary>Другие облака (Render, Streamlit Community Cloud)</summary>

| Сервис | Действия |
|--------|----------|
| **Render** | [render.com](https://render.com) → New → Web Service → репозиторий. Build: `pip install -r requirements.txt`, Start: `streamlit run app.py --server.port=$PORT --server.address=0.0.0.0`. Или Blueprint с `render.yaml`. |
| **Streamlit Cloud** | [share.streamlit.io](https://share.streamlit.io) → Deploy an app → репозиторий, главный файл: `app.py`. |
</details>
...
Подробная инструкция — в разделе **«🌐 Деплой на Railway через GitHub»** выше. Кратко: залейте репозиторий на GitHub → Railway → New Project → Deploy from GitHub repo → выберите репозиторий → Settings → Generate Domain. Переменные окружения не нужны.
